...
static void    handle_thread_error(int status, t_code code)
{
    if(status != 0)
    {
        if(EAGAIN == status)
            ...
            exit_it("the caller does not have appropriate permission to set the scheduling policy and parameters specified in attr");
            ...
            exit_it("no thread could be found corresponding to that specified by thread");
