...
static void	eat(t_philo *philo)
{
	...
	set_long(&philo->philo_mtx, &philo->meals,
		get_long(&philo->philo_mtx, &philo->meals) + 1);
	print_status(philo, EAT, DEBUG_MODE);
	usleep_set(philo->table->time_to_eat, philo->table);
	if (get_long(&philo->philo_mtx, &philo->meals)
...
static void	think(t_philo *philo)
{
	...
	if (philo->table->ph_nbr % 2 == 0)
		return ;
	t_think = philo->table->time_to_eat * 2 - philo->table->time_to_sleep;
	if (t_think < 0)
		t_think = 0;
	usleep_set(t_think * 0.42, philo->table);
}
